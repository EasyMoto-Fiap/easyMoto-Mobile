...
export default function PerfilAdmin() {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      const dadosSalvos = await AsyncStorage.getItem('admin');
      const foto = await AsyncStorage.getItem('fotoAdmin');
  ...
  const escolherImagem = () => {
    Alert.alert('Imagem de perfil', 'Escolha uma opção', [
      {
        text: 'Galeria',
        onPress: async () => {
          ...
          if (!result.canceled) {
            ...
            setImagem(uri);
            await AsyncStorage.setItem('fotoAdmin', uri);
          }
        },
      },
      {
  ...
  const salvarAlteracoes = async () => {
    ...
    await AsyncStorage.setItem('admin', JSON.stringify(dadosAtualizados));
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <Text style={[styles.name, { color: themeColors.text }]}>{nome || 'Administrador'}</Text>
      <Text style={[styles.role, { color: themeColors.text }]}>Administrador</Text>
