...
import { obterUsuarioPorId, atualizarUsuario, deletarUsuario, listarFiliais, Usuario, UpdateUsuarioPayload } from '../services/usuarios';
...
export default function Perfil() {
    ...
    Alert.alert('Selecionar imagem', 'Escolha uma opção', [
      {
        ...
        onPress: async () => {
          ...
          if (!result.canceled) { const uri = result.assets[0].uri; setImagem(uri); await AsyncStorage.setItem('fotoPerfil', uri); }
        }
      },
      {
        text: 'Câmera',
  ...
  const salvarAlteracoes = async () => {
    ...
    if (!nome.trim()) { Alert.alert('Nome inválido', 'Informe seu nome completo.'); return; }
    if (!email.includes('@')) { Alert.alert('Email inválido', 'Informe um email válido.'); return; }
    if (!/^\d{11}$/.test(cpfLimpo)) { Alert.alert('CPF inválido', 'Informe 11 dígitos.'); return; }
    if (!/^\d{10,11}$/.test(telLimpo)) { Alert.alert('Telefone inválido', 'Informe DDD + número.'); return; }
  ...
  return (
    <View style={[styles.container, { backgroundColor: themeColors.background }]}>
      ...
      <Text style={[styles.role, { color: themeColors.text }]}>{perfil === 1 ? 'Administrador' : 'Operador'}</Text>
      ...
      <TextInput style={styles.input} editable={editando} value={telefone} onChangeText={(v) => setTelefone(formatarTelefone(v))} placeholder="Telefone" keyboardType="phone-pad" />
      <TextInput style={styles.input} editable={editando} value={cpf} onChangeText={(v) => setCpf(formatarCPF(v))} placeholder="CPF" />
...
const styles = StyleSheet.create({
  ...
  deleteOutline: { marginTop: 12, padding: 12, borderRadius: 10, width: '100%', alignItems: 'center', borderWidth: 1, borderColor: '#B00020' },
