...
import type { RootStackParamList } from '../navigation/RootNavigator';
    ...
    export default function RegistroDasMotos() {
    const route = useRoute<RouteProp<RootStackParamList, 'RegistroDasMotos'>>();
    const { theme } = useContext(ThemeContext);
    const isDark = theme === 'dark';
    const themeColors = isDark ? colors.dark : colors.light;

    const origem = route?.params?.origem ?? 'painel';
    ...
    function adicionarOuEditarMoto() {
        if (editandoId) {
        setMotos((prev) => prev.map((m) => (m.id === editandoId ? { ...m, ...novaMoto } as Moto : m)));
